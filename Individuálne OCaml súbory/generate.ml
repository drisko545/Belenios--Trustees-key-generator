...
let upload_custom_key ~token ~url =
  let open (val !Belenios_js.I18n.gettext) in
  let container = Dom_html.createDiv document in
  let loaded_public_key = ref None in
  let status = Dom_html.createDiv document in

  let set_status ~color message =
    (* Stavovy text sluzi ako okamzita spatna vazba pre pouzivatela. *)
    status##.style##.color := Js.string color;
    status##.textContent := Js.some @@ Js.string message
  in
  
  (* Tento input prijima lokalny subor s verejnym klucom v JSON formate. *)
  let file_input = Dom_html.createInput document in
  file_input##setAttribute (Js.string "type") (Js.string "file");
  (* Pripustame viacero pripon a MIME typov, aby upload fungoval aj z roznych nastrojov. *)
  file_input##.accept := Js.string ".pubkey,application/json,text/plain";
  file_input##.style##.marginTop := Js.string "8px";
  
  (* Tlacidlo je aktivne az po uspesnej validacii obsahu suboru. *)
  let submit = Dom_html.createButton document in
  submit##.id := Js.string "submit_custom_public_key";
  submit##.disabled := Js._true;
  submit##.textContent := Js.some @@ Js.string @@ s_ "Submit custom public key";
  ...
  let () =
    ...
    file_input##.onchange :=
      Dom_html.handler (fun _ ->
          let$ files = file_input##.files in
          let& file = files##item 0 in
          let reader = new%js File.fileReader in
          reader##.onload :=
            Dom.handler (fun _ ->
                (* FileReader nacita obsah suboru ako text a my ho dalej overime. *)
                let& content = File.CoerceTo.string reader##.result in
                let public_key_text = Js.to_string content in
                let is_valid =
                  try
                    let _ = Yojson.Safe.from_string public_key_text in
                    true
                  with _ -> false
                in
                if is_valid then (
                  (* Po uspesnej validacii ulozime obsah do premennej a povolime odoslanie. *)
                  loaded_public_key := Some public_key_text;
                  submit##.disabled := Js._false;
                  set_status ~color:"green"
                    (s_ "Public key file loaded successfully. You can now submit it."))
                else (
                  (* Pri neplatnom JSONe vraciame formular do bezpecneho stavu. *)
                  loaded_public_key := None;
                  submit##.disabled := Js._true;
                  set_status ~color:"red"
                    (s_ "Invalid JSON format in selected file!"));
                Js._false);
  ...
  let () =
    submit##.onclick :=
      let@ () = lwt_handler in
      (* Submit je mozne pouzit az po uspesnom nacitani a validacii verejneho kluca. *)
      match !loaded_public_key with
      | None ->
          set_status ~color:"red"
            (s_ "Please select a valid .pubkey file first.");
          Lwt.return_unit
      | Some public_key_text ->
          (* Posleme obsah custom kluca na trustee API a cakame na vysledok registracie. *)
          Dom.removeChild container submit;
          file_input##.disabled := Js._true;
          let* x = Api.(post url (`Trustee token) public_key_text) in
          let msg =
            match x.code with
            | 200 -> s_ "Custom public key registration succeeded!"
            | _ -> s_ "Public key registration failed! Check format and try again."
          in
          (* UI zobrazi jasnu spatnu vazbu o uspechu alebo zlyhani registracie. *)
          let element =
            div
              ~a:[ a_id "success"; a_style "color:green; margin-top:10px; font-weight:bold;" ]
              [ txt msg ]
          in
  ...
  let elements = [
    div [ b [ txt @@ s_ "Upload your own public key:" ] ];
    div ~a:[a_style "margin-top:10px;"] [ 
      txt @@ s_ "Select your .pubkey file (generated by generate_ed25519_trustee_key.py):" 
    ];
  ] in
  ...
  List.iter
    ...
    elements;
  Dom.appendChild container (Dom_html.createBr document);
  Dom.appendChild container file_input;
  Dom.appendChild container status;
  Dom.appendChild container (Dom_html.createBr document);
...
let generate_key ~token ~url generate =
  ...
  let choice_div = Dom_html.createDiv document in
  
  let btn_generate = Dom_html.createButton document in
  btn_generate##.id := Js.string "btn_generate_new";
  btn_generate##.textContent := Js.some @@ Js.string @@ s_ "Generate new key";
  btn_generate##.style##.marginRight := Js.string "10px";
  
  let btn_upload = Dom_html.createButton document in
  btn_upload##.id := Js.string "btn_upload_custom";
  btn_upload##.textContent := Js.some @@ Js.string @@ s_ "Upload custom key";
  
  (* Vetva generate vytvori novy kluc, ulozi ho do downloadu a potom odosle verejnu cast. *)
  let () =
    btn_generate##.onclick :=
      let@ () = lwt_handler in
      (* Pri generovani kluca najprv odstranime uvodnu volbu, aby UI ukazalo dalsi krok. *)
      Dom.removeChild container choice_div;
      let* p = generate () in
      let submit =
        ...
        let () =
          r##.onclick :=
            let@ () = lwt_handler in
            (* Toto je standardna vetva pre upload verejneho kluca po jeho lokalnom vytvoreni. *)
      ...
      in
      let download =
        (* Private key sa neodosiela na server; pouzivatel si ho stiahne a uschova lokalne. *)
        ...
        r##.onclick :=
          Dom_html.handler (fun _ ->
              (* Kliknutie povoli odoslanie verejneho kluca v dalsej akcii. *)
              submit##.disabled := Js._false;
              Js._true);
        element
      in
  ...
  (* Handler for Upload custom key button *)
  let () =
    btn_upload##.onclick :=
      let@ () = lwt_handler in
      (* Druha vetva umoznuje upload uz hotoveho verejneho kluca bez generovania v UI. *)
      Dom.removeChild container choice_div;
      let* custom_ui = upload_custom_key ~token ~url in
      List.iter
  ...
  (* Add choice buttons to UI *)
  let intro_text = div ~a:[a_style "margin-bottom:15px;"] [ 
    b [ txt @@ s_ "Choose an option:" ] 
  ] in
  Dom.appendChild choice_div (Tyxml_js.To_dom.of_div intro_text);
  Dom.appendChild choice_div btn_generate;
  Dom.appendChild choice_div (Dom_html.createBr document);
  Dom.appendChild choice_div (Dom_html.createBr document);
  Dom.appendChild choice_div btn_upload;
  Dom.appendChild container choice_div;
